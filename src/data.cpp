...
const std::string arrayType = "CTable";
const std::string dataClassPtr = "Ptr";
...
Schema convert(const ast::Schema &ast)
{
    ...
    {
        while (ast.types.count(name))
            name = ast.types.find(name)->second;
        return name;
    };
...
ModuleContext Schema::printHelpers() const
{
    ...
    // getTableType()
    {
        ...
        mc.cpp.beginBlock();
        for (auto &c : classes({ fInline }, true))
            mc.cpp.addLine("{ \"" + c.getHumanName() + "\", EObjectType::" + c.getCppName() + " },");
    ...
    // getTableNameByType()
    {
        ...
        mc.cpp.beginBlock();
        for (auto &c : classes({ fInline }, true))
            mc.cpp.addLine("{ EObjectType::" + c.getCppName() + ", \"" + c.getHumanName() + "\" },");
        mc.cpp.endBlock(true);
        mc.cpp.addLine();
        ...
        // retranslateFieldNames()
        {
            ...
            for (auto &c : classes)
                for (auto &v : c.getVariables())
                    vars.insert(v);
            for (auto &v : vars)
                mc.cpp.addLine("{ \"" + v.getName() + "\", " + tr(v.getName()) + " },");
            mc.cpp.endBlock(true);
            mc.cpp.addLine("fieldNames = names;");
            mc.cpp.endFunction();
        }
...
ModuleContext Schema::printTypes() const
{
    ...
    std::sort(cls.begin(), cls.end(), [](const auto &c1, const auto &c2)
    {
        auto v1 = c1.getVariables(true);
        for (auto &v : v1)
        {
            if (v.getType()->getCppName() == c2.getCppName())
                return false;
        }
        auto v2 = c2.getVariables(true);
        for (auto &v : v2)
        {
            if (v.getType()->getCppName() == c1.getCppName())
                return true;
        }
        return false;
    });
...
ModuleContext Schema::printStorage() const
{
    ...
    for (auto &c : cls)
        mc.hpp.addLine(arrayType + "<" + c.getCppName() + "> " + c.getCppArrayVariableName() + ";");
    mc.hpp.addLine();
    mc.hpp.addLineNoSpace("public:");
...
ModuleContext Class::print() const
{
        ...
        if (getCppName() != String)
        {
            ...
            for (auto &nameType : namesOrder)
            {
                ...
                case ObjectName::Parent:
                    if (flags[fProxy])
                    {
                        mc.cpp.addLine("s = to_string(" + parent->getCppVariableName() + ");");
                        checkReturn();
                    }
                    break;
                }
    ...
    // operator->()
    {
        if (flags[fProxy])
        {
            ...
            mc.cpp.addLine("if (" + parent->getCppVariableName() + ")");
            mc.cpp.increaseIndent();
            mc.cpp.addLine("return " + parent->getCppVariableName() + ";");
            mc.cpp.decreaseIndent();
...
ModuleContext Class::printIo() const
{
    ...
    // printLoadPtrs()
    {
        ...
        if (fks)
        {
            ...
            for (auto &v : vars)
            {
                ...
                if (v.isFk())
                {
                    std::string name2 = v.getType()->getCppVariableName();
                    mc.cpp.addLine("if (" + name2 + ".find(" + var + "->" + v.getName() + idAccess + ") != " + name2 + ".end())");
                    mc.cpp.increaseIndent();
                    mc.cpp.addLine(var + "->" + v.getName() + " = " +
                        name2 + "[" + var + "->" + v.getName() + idAccess + "];");
                    mc.cpp.decreaseIndent();
                }
            }
            mc.cpp.endBlock();
    ...
    // printLoadArrays()
    {
        ...
        if (!vars_container.empty())
        {
            ...
            for (auto &v : vars_container)
            {
                ...
                mc.cpp.increaseIndent();
                mc.cpp.addLine("if (" + getCppVariableName() + ".first == " + t->getCppVariableName() + "->" + getCppVariableName() + idAccess + ")");
                mc.cpp.increaseIndent();
                mc.cpp.addLine(getCppVariableName() + "->" + t->getCppArrayVariableName() + ".insert(" + t->getCppVariableName() + ");");
                mc.cpp.decreaseIndent();
                mc.cpp.decreaseIndent();
            }
            mc.cpp.endBlock();
    ...
    // printSave()
    {
        ...
        for (auto &v : vars)
            mc.cpp.addLine("ret = " + v.printSaveSqlite3("v") + ";");
...
ModuleContext Class::printAddDeleteRecord() const
{
    ...
    // add record
    {
        ...
        else
        {
            ...
            {
                std::string var = parent->getCppVariableName();
                mc.cpp.addLine("auto v = " + getCppVariableName() + ".createAtEnd();");
                mc.cpp.addLine(parent->getCppName() + " *" + var + " = (" + parent->getCppName() + " *)" + param_name + ";");
                mc.cpp.addLine(var + "->" + parent->getCppArrayVariableName() + ".insert(v);");
                mc.cpp.addLine("v->" + var + " = " + parent->getCppArrayVariableName() + "[" + var + "->id];");
                mc.cpp.addLine("return v;");
            }
        }
        mc.cpp.endFunction();
    ...
    // delete record
    {
        ...
        mc.cpp.addLine(getCppVariableName() + ".erase(v->id);");
        mc.cpp.endFunction();
    }
...
std::string Variable::print() const
{
    ...
    if (isFk())
    {
        ...
        //    " = std::make_shared<" + type->getCppName() + ">()";
        s += "" + dataClassPtr + "<" + type->getCppName() + "> " + getName();
    }
    else
    {
...
std::string Variable::printLoadSqlite3(std::string var) const
{
    ...
    std::string s;
    s += var + "->" + (isFk() ? getName() + idAccess : getName()) + " = ";
    switch (t)
    {
    case DataType::Integer:
