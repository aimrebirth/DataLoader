...
string Table::print(const Tables &tables, string &si) const
{
    ...
    if (container)
        s += "\n";
    for (auto &t : contains)
        s += space + arrayType + "<" + t->getCppName() + "> " + t->getChildVariableArrayName(getCppName()) + ";\n";
    s += "\n";
    ...
    si += "{\n";
    {
        ...
        if (container)
        {
            ...
            for (auto &t : contains)
            {
                ...
                si += space + "root->sortChildren(0, Qt::AscendingOrder);\n";
            }
            si += "\n";
        }
    ...
    s += space + "template <class T> friend class CTable;\n";
    s += "\n";
    ...
    // type
    s += space + "static const EObjectType object_type = EObjectType::" + getCppName() + ";\n";
    s += space + "static const bool has_id = " + (hasIdField ? "true" : "false") + ";\n";
    s += "\n";
...
string Table::printIo(const Tables &tables, string &impl) const
{
    string s;
    s += printLoad(impl);
...
string Table::printLoad(string &si) const
{
    string s;
    ...
    {
        string space = ::space + ::space;
        si += space + "auto v = " + getVariableArrayName() + ".create();" + "\n";
        auto cols = mapToSet(columns);
        for (auto &col : cols)
            si += space + col.printLoadSqlite3("v") + ";\n";
        si += space + getVariableArrayName() + ".insert(v);" + "\n";
    }
    si += space + "}\n";
...
string Table::printLoadPtrs(string &si) const
{
    string s;
...
string Table::printLoadArrays(const Tables &tables, string &si) const
{
    string s;
        ...
        if (container)
        {
            ...
            for (auto &table : contains)
            {
                ...
                si += space + "for (auto &" + table->getVariableName() + " : " + table->getVariableArrayName() + ")\n";
                {
                    ...
                    si += space + "if (" + getVariableName() + ".first == " + table->getVariableName() + "->" + getVariableName() + ".id)\n";
                    {
                        string space = ::space + ::space + ::space + ::space;
                        si += space + getVariableName() + "->" + table->getChildVariableArrayName(table->getParentName()) + ".insert(" + table->getVariableName() + ");\n";
                    }
                }
            }
            si += space + "}\n";
...
string Table::printSave(string &si) const
{
    string s;
...
string Table::printAddDeleteRecordVirtual() const
{
    string s;
    ...
    s += space + "virtual Ptr<" + getCppName() + "> add" + getCppName() + "(" + param_h + ") = 0;\n";
    s += space + "virtual void delete" + getCppName() + "(" + getCppName() + " *object) = 0;\n";

    return s;
}

string Table::printAddDeleteRecord(string &si) const
{
    string s;
    ...
    // add record
    s += space + "virtual Ptr<" + getCppName() + "> add" + getCppName() + "(" + param_h + ");\n";
    si += "Ptr<" + getCppName() + "> " + storageImpl + "::add" + getCppName() + "(" + param + ")\n";
    si += "{\n";
    if (isMapTable)
    {
        si += space + "auto v = " + getVariableArrayName() + ".createAtEnd();" + "\n";

        string var = getParentVariableName();
        si += space + getParentName() + " *" + var + " = (" + getParentName() + " *)" + param_name + ";" + "\n";
        si += space + var + "->" + getChildVariableArrayName(getParentName()) + ".insert(v);" + "\n";
    ...
    else
    {
        ...
        {
            si += space + "return " + getVariableArrayName() + ".createAtEnd();" + "\n";
        }
        else
        {
            string var = getParentVariableName();
    ...
    {
        si += space + getVariableArrayName() + ".erase(v);\n";
    }
    si += "}\n\n";
    return s;
}
...
string Database::printStorage(string &si)
{
    ...
    s += "public:\n";
    for (auto &table : tables)
        s += space + arrayType + "<" + table.second.getCppName() + "> " + table.second.getVariableArrayName() + ";\n";
    s += "\n";
    s += "public:\n";
    s += space + "Storage();\n";
    si += "Storage::Storage()\n";
    si += "{\n";
    for (auto &table : tables)
        si += space + table.second.getVariableArrayName() + ".name = \"" + table.second.getCppName() + "\";\n";
    si += "}\n";
    si += "\n";
    s += space + "virtual ~Storage();\n";
    si += "Storage::~Storage()\n";
    si += "{\n";
    si += "}\n";
    s += "\n";
    ...
    s += "#endif\n";
    s += "\n";
    s += space + "virtual OrderedObjectMap getOrderedMap(EObjectType type, std::function<bool(Ptr<IObject>)> f = std::function<bool(Ptr<IObject>)>()) const = 0;\n";
    s += "\n";
...
string Database::printAddDeleteRecordVirtual()
{
    string s;
    for (auto &table : tables)
        s += table.second.printAddDeleteRecordVirtual();
    s += "\n";
...
string Database::printAddDeleteRecord(string &si)
{
    ...
    for (auto &table : tables)
    {
        ...
        si += space + space + "return add" + table.second.getCppName() + "(" + (table.second.hasParentTable() ? "parent" : "") + ");" + "\n";
...
string Database::printQtTreeView(string &si)
{
    ...
    for (auto &table : tables)
    {
        si += ::space + "case EObjectType::" + table.second.getCppName() + ":" + "\n";
        string space = ::space + ::space;
        si += space + "return add" + table.second.getCppName() + "(" + (table.second.hasParentTable() ? "parent" : "") + ")->printQtTreeView(item);" + "\n";
    }
    si += space + "default:" + "\n";
...
string Database::printGetOrderedMap(string &si)
{
    string s;
    s += "\n";
    s += space + "virtual OrderedObjectMap getOrderedMap(EObjectType type, std::function<bool(Ptr<IObject>)> f = std::function<bool(Ptr<IObject>)>()) const;\n";
    si += "OrderedObjectMap " + storageImpl + "::getOrderedMap(EObjectType type, std::function<bool(Ptr<IObject>)> f) const\n";
    si += "{\n";
    si += space + "switch (type)\n";
    si += space + "{\n";
    for (auto &table : tables)
